  ...
  db.all(
    `
      SELECT
        ...
        pgw.percentile_grades_offense_penalty AS percentile_grades_offense_penalty_rushing, pdw.percentile_grades_offense_penalty AS percentile_grades_offense_penalty_receiving,
...
app.get('/api/teams_roster/:id/:year', (req, res) => {
    ...
    db.all('SELECT playerId, name, position, school, teamID, height, weight, jersey, headshotURL, homeCity, homeState FROM Players_Basic WHERE teamID = ? AND year = ? AND jersey != ?', [idNum, yearNum, '-1'], (err, rows) => {
        if (err) {
            console.error('Database error:', err.message);
            return res.status(500).json({ error: 'Internal server error' });
        }
