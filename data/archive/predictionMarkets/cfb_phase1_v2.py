#!/usr/bin/env python3
"""
CFB Enhanced Model V2
FIXES:
- Proper conviction calculation (was showing 1.00 for everything)
- Upset detection features
- Team consistency/variance metrics
- Better handling of close games
"""
...
print("="*80)
print("CFB ENHANCED MODEL V2 - FIXED CONVICTION + UPSET DETECTION")
print("="*80)
...
def load_year_data(year):
    ...
    # Remove duplicates (noticed in analysis)
...
def get_team_consistency(tid, week, feature, stats):
    """NEW: Measure team consistency (lower std = more predictable)"""
    team_games = stats[(stats["team_id"] == tid) & (stats["week"] < week)].tail(5)
    if len(team_games) >= 3 and feature in team_games.columns:
        values = pd.to_numeric(team_games[feature], errors='coerce').fillna(0)
        return float(values.std()) if len(values) > 0 else 0.0
    return 0.0

def compute_trend(tid, week, feature, stats):
    """Detect if team trending up/down"""
    if feature not in stats.columns:
        return 0.0
    team_games = stats[(stats["team_id"] == tid) & (stats["week"] < week)].tail(5)
    if len(team_games) >= 3:
        values = pd.to_numeric(team_games[feature], errors='coerce').fillna(0).values
...
def build_enhanced_features_v2(g, home_id, away_id, week, baseline_features, 
                               rolling_stats, stats, elo_by_week, year):
    """V2: Add consistency, upset potential, better close game handling"""
    feature_dict = {}
    ...
    # NEW: Consistency features (key for close games!)
    h_consistency = get_team_consistency(home_id, week, "points", stats)
    a_consistency = get_team_consistency(away_id, week, "points", stats)
    feature_dict["home_consistency"] = h_consistency
    feature_dict["away_consistency"] = a_consistency
    feature_dict["consistency_diff"] = a_consistency - h_consistency  # Higher = away more volatile
    
    # NEW: Trend features (hot/cold teams)
    h_trend = compute_trend(home_id, week, "points", stats)
    a_trend = compute_trend(away_id, week, "points", stats)
    feature_dict["home_trend"] = h_trend
    feature_dict["away_trend"] = a_trend
    feature_dict["trend_diff"] = h_trend - a_trend
    
    # NEW: Recent performance variance
    h_def_consistency = get_team_consistency(home_id, week, "defense_ppa", stats)
    a_def_consistency = get_team_consistency(away_id, week, "defense_ppa", stats)
    feature_dict["def_consistency_diff"] = h_def_consistency - a_def_consistency
    ...
    # Vegas lines (CRITICAL)
    if pd.notna(g.spread):
        feature_dict["vegas_spread"] = float(g.spread)
        feature_dict["spread_magnitude"] = abs(float(g.spread))  # NEW: How confident is Vegas?
        feature_dict["is_close_game"] = 1.0 if abs(float(g.spread)) < 7 else 0.0  # NEW: Flag close games
    ...
    if pd.notna(g.homeML) and pd.notna(g.awayML):
        ...
        if vegas_home_prob is not None:
            ...
            # NEW: Vegas confidence (how far from 50/50?)
            feature_dict["vegas_confidence"] = abs(vegas_home_prob - 0.5) * 2
...
log_progress("Building training features (V2)...")
train_X = []
train_y = []
...
for target_week in TRAIN_WEEKS:
    ...
    for _, g in iterator:
        ...
        feats = build_enhanced_features_v2(
...
log_progress(f"NEW FEATURES: consistency, trends, close_game_flag, vegas_confidence")

# Train model
log_progress("Training LightGBM V2...")
...
log_progress(f"✓ Model trained in {train_time:.1f} seconds")

# Calibrate
log_progress("Calibrating probabilities...")
calibrated_model = CalibratedClassifierCV(model, method='isotonic', cv='prefit')
calibrated_model.fit(X_train, y_train)

# Test
log_progress(f"Testing on weeks {TEST_WEEKS[0]}-{TEST_WEEKS[-1]}...")
test_predictions = []
...
for target_week in TEST_WEEKS:
    ...
    for _, g in iterator:
        ...
        win_prob = float(probs[1])  # Probability of home win
        
        # FIXED CONVICTION CALCULATION
        # Conviction = distance from 50% (0 to 1 scale)
...
if len(completed) > 0:
    ...
    for week in TEST_WEEKS:
        ...
        if len(week_preds) > 0:
            ...
            # Week 11 breakdown
            if week == 11:
                close = week_preds[week_preds['spread'].abs() < 7]
                print(f"     Close games (<7): {close['correct'].mean()*100:.1f}% ({len(close)} games)")

# Save
cursor = conn.cursor()
cursor.execute("DROP TABLE IF EXISTS Teams_Games_Enhanced_V2_Predictions")
cursor.execute("""
    CREATE TABLE Teams_Games_Enhanced_V2_Predictions (
...
cursor.executemany("""
    INSERT OR REPLACE INTO Teams_Games_Enhanced_V2_Predictions VALUES (?, ?, ?, ?, ?, ?, ?, ?)
""", [(
