...
# Define table for Full Percentiles (WR) data
...
try:
    ...
    # Fetch existing player data from Players_Basic with PFF ID and teamID for WRs
    cursor.execute("SELECT playerId, player_id_PFF, name, team, teamID FROM Players_Basic WHERE position IN ('WR')")
    ...
    for col in BASE_METRIC_COLS - existing_cols:
        if col not in {'playerId', 'year', 'name', 'team', 'teamID', 'targets', 'percentile_targets'}:
            try:
