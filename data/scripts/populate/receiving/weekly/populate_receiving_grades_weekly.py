...
# Define table for Receiving Grades Weekly data
TABLE_NAME = "Players_ReceivingGrades_Weekly"
...
# Load Receiving Grades weekly data and dynamically build all columns
pff_data = {}
BASE_DIR = Path("/Users/christianberry/Desktop/Perennial Data/perennial-data-app/data/PFF_Data/Receiving/WeeklyReports/ReceivingGrades")
if not BASE_DIR.exists():
    exit(1)
for csv_file in BASE_DIR.glob("*_ReceivingGrades.csv"):
    file_parts = csv_file.stem.split('_')
    if len(file_parts) >= 2 and file_parts[-1] == "ReceivingGrades":
        try:
        ...
        try:
            with open(csv_file, 'r', encoding='utf-8') as csvfile:
                ...
                BASE_METRIC_COLS.update(all_cols - {'playerId', 'year', 'week', 'seasonType', 'player', 'team', 'player_game_count'})
                for row in reader:
