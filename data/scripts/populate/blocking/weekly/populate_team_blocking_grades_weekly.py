...
# Step 1: Dynamically fetch all columns from Players_BlockingGrades_Weekly
print("\nStep 1: Fetching table schema...")
cursor.execute("PRAGMA table_info(Players_BlockingGrades_Weekly)")
columns = [row[1] for row in cursor.execute("PRAGMA table_info(Players_BlockingGrades_Weekly)")]
print(f"Step 1: Total columns in Players_BlockingGrades_Weekly: {len(columns)}")
...
# Qualify all columns from Players_BlockingGrades_Weekly with table alias 'b' and add tg columns
...
agg_overrides = {
    ...
    'block_percent':lambda x: (x * df.loc[x.index, 'snap_counts_block']).sum() / df.loc[x.index, 'snap_counts_block'].sum() if df.loc[x.index, 'snap_counts_block'].sum() > 0 else 0,
...
# Step 4: Generate SQL to create and populate Team_BlockingGrades_Weekly
print("\nStep 4: Creating and populating SQL table...")
drop_table_sql = """
DROP TABLE IF EXISTS Team_BlockingGrades_Weekly;
"""
...
create_table_sql = f"""
CREATE TABLE Team_BlockingGrades_Weekly (
...
team_df.to_sql('Team_BlockingGrades_Weekly', conn, if_exists='replace', index=False, dtype=to_sql_dtype)
