...
COLUMNS_TO_EXCLUDE = [
    'position',
    'player_id',
    'team_name', 'franchise_id'
    # Add any other columns you want to exclude here
]
# ============================================================================

# Fetch all columns from Players_BlockingGrades_Weekly
...
snap_count_cols = ['snap_counts_offense', 'snap_counts_pass_block', 'snap_counts_run_block', 'snap_counts_block']
...
weighted_avg_configs = [
    ('block_percent', 'snap_counts_block'),
    ('grades_offense', 'snap_counts_offense'),
    ('grades_offense_adjusted', 'snap_counts_offense'),
    ...
    ('grades_run_block_adjusted', 'snap_counts_run_block')
]
...
cursor.execute("DROP TABLE IF EXISTS Team_BlockingGrades_Weekly;")
...
print(f"✓ Team_BlockingGrades_Weekly table created with {len(team_df)} records ({year_range})")
if excluded_found:
