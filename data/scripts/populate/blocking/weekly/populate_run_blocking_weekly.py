...
# Define table for Blocking Run Weekly data
TABLE_NAME = "Players_BlockingRun_Weekly"
...
# Load Blocking Run weekly data and dynamically build all columns
pff_data = {}
BASE_DIR = Path("/Users/christianberry/Desktop/Perennial Data/perennial-data-app/data/PFF_Data/Blocking/WeeklyReports/RunBlocking")
if not BASE_DIR.exists():
    print(f"Error: Directory {BASE_DIR} does not exist. Please check the path.")
    exit(1)
else:
    for csv_file in BASE_DIR.glob("*_RunBlocking.csv"):
        file_parts = csv_file.stem.split('_')
        if len(file_parts) >= 2 and file_parts[-1] == "RunBlocking":
            try:
