...
# Step 1: Dynamically fetch all columns from Players_BlockingRun_Weekly
...
print(f"Step 1: Total columns in Players_BlockingRun_Weekly: {len(columns)}")
...
# Qualify all columns from Players_BlockingRun_Weekly with table alias 'b' and add tg columns
...
agg_overrides = {
    ...
    'gap_grades_run_block':lambda x: (x * df.loc[x.index, 'gap_snap_counts_run_play']).sum() / df.loc[x.index, 'gap_snap_counts_run_play'].sum() if df.loc[x.index, 'gap_snap_counts_run_play'].sum() > 0 else 0,
    'gap_grades_run_block_adjusted':lambda x: (x * df.loc[x.index, 'gap_snap_counts_run_play']).sum() / df.loc[x.index, 'gap_snap_counts_run_play'].sum() if df.loc[x.index, 'gap_snap_counts_run_play'].sum() > 0 else 0,
    'grades_run_block_adjusted':lambda x: (x * df.loc[x.index, 'snap_counts_run_play']).sum() / df.loc[x.index, 'snap_counts_run_play'].sum() if df.loc[x.index, 'snap_counts_run_play'].sum() > 0 else 0,
    'zone_grades_run_block':lambda x: (x * df.loc[x.index, 'zone_snap_counts_run_play']).sum() / df.loc[x.index, 'zone_snap_counts_run_play'].sum() if df.loc[x.index, 'zone_snap_counts_run_play'].sum() > 0 else 0,
    'zone_grades_run_block_adjusted':lambda x: (x * df.loc[x.index, 'zone_snap_counts_run_play']).sum() / df.loc[x.index, 'zone_snap_counts_run_play'].sum() if df.loc[x.index, 'zone_snap_counts_run_play'].sum() > 0 else 0,
...
# Step 4: Generate SQL to create and populate Team_RunBlocking_Weekly
print("\nStep 4: Creating and populating SQL table...")
drop_table_sql = """
DROP TABLE IF EXISTS Team_RunBlocking_Weekly;
"""
