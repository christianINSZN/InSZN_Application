...
# Step 1: Dynamically fetch all columns from Players_BlockingPass_Weekly
print("\nStep 1: Fetching table schema...")
cursor.execute("PRAGMA table_info(Players_BlockingPass_Weekly)")
columns = [row[1] for row in cursor.execute("PRAGMA table_info(Players_BlockingPass_Weekly)")]
print(f"Step 1: Total columns in Players_BlockingPass_Weekly: {len(columns)}")
...
# Qualify all columns from Players_BlockingPass_Weekly with table alias 'b' and add tg columns
...
query = f"""
    ...
    FROM Players_BlockingPass_Weekly b
...
agg_overrides = {
    ...
    'non_spike_pass_block_percentage':lambda x: (x * df.loc[x.index, 'snap_counts_pass_block']).sum() / df.loc[x.index, 'snap_counts_pass_block'].sum() if df.loc[x.index, 'snap_counts_pass_block'].sum() > 0 else 0,
    ...
    'true_pass_set_grades_pass_block':lambda x: (x * df.loc[x.index, 'true_pass_set_snap_counts_pass_play']).sum() / df.loc[x.index, 'true_pass_set_snap_counts_pass_play'].sum() if df.loc[x.index, 'true_pass_set_snap_counts_pass_play'].sum() > 0 else 0,
    'true_pass_set_grades_pass_block_adjusted':lambda x: (x * df.loc[x.index, 'true_pass_set_snap_counts_pass_play']).sum() / df.loc[x.index, 'true_pass_set_snap_counts_pass_play'].sum() if df.loc[x.index, 'true_pass_set_snap_counts_pass_play'].sum() > 0 else 0,
    'true_pass_set_non_spike_pass_block_percentage':lambda x: (x * df.loc[x.index, 'true_pass_set_snap_counts_pass_play']).sum() / df.loc[x.index, 'true_pass_set_snap_counts_pass_play'].sum() if df.loc[x.index, 'true_pass_set_snap_counts_pass_play'].sum() > 0 else 0,
    'true_pass_set_pass_block_percent':lambda x: (x * df.loc[x.index, 'true_pass_set_snap_counts_pass_play']).sum() / df.loc[x.index, 'true_pass_set_snap_counts_pass_play'].sum() if df.loc[x.index, 'true_pass_set_snap_counts_pass_play'].sum() > 0 else 0,
    'true_pass_set_pbe':lambda x: (x * df.loc[x.index, 'true_pass_set_snap_counts_pass_play']).sum() / df.loc[x.index, 'true_pass_set_snap_counts_pass_play'].sum() if df.loc[x.index, 'true_pass_set_snap_counts_pass_play'].sum() > 0 else 0,
...
# Step 4: Generate SQL to create and populate Team_PassBlocking_Weekly
print("\nStep 4: Creating and populating SQL table...")
drop_table_sql = """
DROP TABLE IF EXISTS Team_PassBlocking_Weekly;
"""
...
create_table_sql = f"""
CREATE TABLE Team_PassBlocking_Weekly (
...
team_df.to_sql('Team_PassBlocking_Weekly', conn, if_exists='replace', index=False, dtype=to_sql_dtype)
