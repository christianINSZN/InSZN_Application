...
# Define table for Blocking Pass Weekly data
TABLE_NAME = "Players_BlockingPass_Weekly"
BASE_METRIC_COLS = set()

# Fetch existing player data from Players_Basic with PFF ID and teamID for blocking positions
cursor.execute("SELECT playerId, player_id_PFF, name, team, teamID FROM Players_Basic WHERE position IN ('RB', 'TE', 'G', 'C', 'T', 'OT', 'OG', 'OC', 'OL')")
...
# Load Blocking Pass weekly data and dynamically build all columns
pff_data = {}
BASE_DIR = Path("/Users/christianberry/Desktop/Perennial Data/perennial-data-app/data/PFF_Data/Blocking/WeeklyReports/PassBlocking")
if not BASE_DIR.exists():
    print(f"Error: Directory {BASE_DIR} does not exist. Please check the path.")
    exit(1)
else:
    for csv_file in BASE_DIR.glob("*_PassBlocking.csv"):
        file_parts = csv_file.stem.split('_')
        if len(file_parts) >= 2 and file_parts[-1] == "PassBlocking":
            try:
                ...
                if game_type.isdigit():
                    ...
                    week = 15
                    seasonType = 'regular'
                elif game_type == '1stPO':
                    week = 1
                    seasonType = 'postseason'
                elif game_type == '2ndPO':
                    week = 2
                    seasonType = 'postseason'
                elif game_type == '3rdPO':
                    week = 3
                    seasonType = 'postseason'
                elif game_type == '4thPO':
                    week = 4
                    seasonType = 'postseason'
                else:
                ...
                with open(csv_file, 'r', encoding='utf-8') as csvfile:
                    ...
                    for row in reader:
                        ...
                        if pff_id in players_basic:
                            ...
                            metrics = {col: float(row.get(col, 0)) if row.get(col) and row.get(col).replace('.', '').isdigit() else None for col in all_cols}
