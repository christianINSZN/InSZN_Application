...
# Fetch existing player data from Players_Basic with PFF ID and teamID for QBs and RBs
...
# Load Rushing Grades weekly data and dynamically build metric columns
pff_data = {}
            ...
            with open(csv_file, 'r', encoding='utf-8') as csvfile:
                ...
                for row in reader:
                    ...
                    if pff_id in players_basic:
                        ...
                        metrics = {col: float(row.get(col)) if row.get(col) and row.get(col).replace('.', '').isdigit() else None for col in metric_cols}
...
""", (40,))  # National average as fallback
