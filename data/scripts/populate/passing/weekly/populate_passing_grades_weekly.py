...
# Load Passing Concept weekly data and dynamically build metric columns
pff_data = {}
...
if BASE_DIR.exists():
    for csv_file in BASE_DIR.glob("*_PassingGrades.csv"):
        file_parts = csv_file.stem.split('_')
        if len(file_parts) >= 2 and file_parts[-1] == "PassingGrades":
            try:
            ...
            with open(csv_file, 'r', encoding='utf-8') as csvfile:
                ...
                if "player_id" not in reader.fieldnames:
                    ...
                    continue
                metric_cols = set(col for col in reader.fieldnames if col not in EXCLUDED_COLS and not col.startswith('grades_') and not col.startswith('no_'))
                metric_cols.update(col for col in reader.fieldnames if col not in EXCLUDED_COLS)
                if not metric_cols:
