...
fbs_team_ids = pd.read_sql_query("""
    SELECT DISTINCT teamId FROM Teams_Rankings
...
# Vegas lines
games["home_spread"] = pd.to_numeric(
...
# Map external teamId → internal team_id
team_id_map = {}
...
for week in sorted(games["week"].unique()):
    ...
    for _, g in week_games.iterrows():
        ...
        res_h = 1 if g.homePoints > g.awayPoints else 0 if g.homePoints < g.awayPoints else 0.5
        upd = 32 * (res_h - exp_h)
...
per_game_feats = [
    ...
    if not c.startswith("cumulative_") and c not in ["game_id", "season", "week", "team_id", "homeAway"]
]
...
for target_week in range(3, 16):
    ...
    for team in train_stats["team_id"].unique():
        df = train_stats[train_stats["team_id"] == team][["week"] + per_game_feats]
    ...
    if hist.empty:
        print(" No completed games for training")
        continue
    ...
    for _, r in hist.iterrows():
        ...
        row.update({
            ...
            "vegas_model_gap": (r["vegas_home_win_prob"] or 0.5) - 0.5
        })
        rows.append(row)
    ...
    # Predict target week
    week_games = games[games["week"] == target_week].copy()
    ...
    for _, g in week_games.iterrows():
        ...
        prev_elo = elo_pre_game.get(target_week - 1, elo_pre_game[0])
        row.update({
            "home_adv": HOME_ADV,
            "elo_diff": prev_elo.get(h_id, 1500) - prev_elo.get(a_id, 1500),
            "home_spread": g.home_spread,
            "vegas_home_win_prob": g.vegas_home_win_prob,
            "vegas_model_gap": (g.vegas_home_win_prob or 0.5) - 0.5
        })

        Xp = pd.DataFrame([row]).reindex(columns=X.columns, fill_value=np.nan)
        Xp_sc = scaler.transform(imp.transform(Xp))
        raw_prob = float(model.predict_proba(Xp_sc)[0][1])

        # VEGAS BLEND — THIS IS THE MONEY
        vegas_prob = g.vegas_home_win_prob
        if pd.isna(vegas_prob):
            final_prob = np.clip(raw_prob, 0.05, 0.95)
        else:
            confidence = abs(raw_prob - 0.5) * 2
            model_weight = 0.45 * confidence
            final_prob = (1 - model_weight) * vegas_prob + model_weight * raw_prob
        ...
        all_predictions.append((
            ...
            float(vegas_prob) if pd.notna(vegas_prob) else None
        ))
...
if weekly_results:
    ...
    print(f"\nOVERALL: {total} games | Accuracy: {overall_acc:.1%} | Avg Brier: {overall_brier:.3f}")