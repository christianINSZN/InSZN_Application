...
# Step 1: Dynamically fetch all columns from Players_DefenseSlotCoverage_Weekly
print("\nStep 1: Fetching table schema...")
cursor.execute("PRAGMA table_info(Players_DefenseSlotCoverage_Weekly)")
columns = [row[1] for row in cursor.execute("PRAGMA table_info(Players_DefenseSlotCoverage_Weekly)")]
print(f"Step 1: Total columns in Players_DefenseSlotCoverage_Weekly: {len(columns)}")
...
# Qualify all columns from Players_DefenseSlotCoverage_Weekly with table alias 'b' and add tg columns
...
query = f"""
    ...
    FROM Players_DefenseSlotCoverage_Weekly b
...
agg_overrides = {
    ...
    'yards_after_catch':lambda x: (x * df.loc[x.index, 'coverage_snaps']).sum() / df.loc[x.index, 'coverage_snaps'].sum() if df.loc[x.index, 'coverage_snaps'].sum() > 0 else 0,
    'qb_rating_against':lambda x: (x * df.loc[x.index, 'coverage_snaps']).sum() / df.loc[x.index, 'coverage_snaps'].sum() if df.loc[x.index, 'coverage_snaps'].sum() > 0 else 0,
    'yards_per_coverage_snap':lambda x: (x * df.loc[x.index, 'coverage_snaps']).sum() / df.loc[x.index, 'coverage_snaps'].sum() if df.loc[x.index, 'coverage_snaps'].sum() > 0 else 0,
...
# Step 4: Generate SQL to create and populate Team_DefenseSlotCoverage_Weekly
print("\nStep 4: Creating and populating SQL table...")
drop_table_sql = """
DROP TABLE IF EXISTS Team_DefenseSlotCoverage_Weekly;
"""
...
create_table_sql = f"""
CREATE TABLE Team_DefenseSlotCoverage_Weekly (
...
team_df.to_sql('Team_DefenseSlotCoverage_Weekly', conn, if_exists='replace', index=False, dtype=to_sql_dtype)
