...
# Load Defense Pass Rush weekly data and dynamically build all columns
pff_data = {}
...
if not BASE_DIR.exists():
    ...
    for csv_file in BASE_DIR.glob("*_PassRush.csv"):  # Match {year}_{week}_PassRush.csv
        file_parts = csv_file.stem.split('_')
        if len(file_parts) >= 2 and file_parts[-1] == "PassRush":
            try:
