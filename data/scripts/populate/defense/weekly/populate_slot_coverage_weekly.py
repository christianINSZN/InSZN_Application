...
# Load Defense Slot Coverage weekly data and dynamically build all columns
pff_data = {}
...
if not BASE_DIR.exists():
    ...
    for csv_file in BASE_DIR.glob("*_SlotCoverage.csv"):  # Match {year}_{week}_SlotCoverage.csv
        file_parts = csv_file.stem.split('_')
        if len(file_parts) >= 2 and file_parts[-1] == "SlotCoverage":
            try:
                ...
                if game_type.isdigit():
                    ...
                    seasonType = 'regular'  # Changed from 'postseason' to 'regular' if appropriate
...
cursor.execute(f"""
    CREATE TABLE IF NOT EXISTS {TABLE_NAME} (
        ...
        opponent_offense_rating REAL{', ' + ', '.join(f'{col}_adjusted REAL' for col in all_grades_cols) if all_grades_cols else ''},
...
for key, data in pff_data.items():
    ...
    base_cols = ['playerId', 'year', 'week', 'seasonType', 'opponentID', 'player', 'team', 'teamID', 'player_game_count']
    all_cols = base_cols + list(BASE_METRIC_COLS) + ['opponent_offense_rating'] + [f"{col}_adjusted" for col in all_grades_cols]
    ...
    # Debug: Print the generated INSERT query
    insert_query = f"""
...
""", (10.0,))  # National average as fallback, matching previous defense table
...
mean_offense_rating = float(mean_offense_rating_row[0]) if mean_offense_rating_row else 10.0  # Use national average as default
