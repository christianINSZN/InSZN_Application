...
# Load Defense Run Defense weekly data and dynamically build all columns
pff_data = {}
...
if not BASE_DIR.exists():
    ...
    for csv_file in BASE_DIR.glob("*_RunDefense.csv"):  # Match {year}_{week}_RunDefense.csv
        file_parts = csv_file.stem.split('_')
        if len(file_parts) >= 2 and file_parts[-1] == "RunDefense":
            try:
