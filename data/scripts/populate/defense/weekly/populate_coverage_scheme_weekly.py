...
# Load Defense Coverage Scheme weekly data and dynamically build all columns
pff_data = {}
...
if not BASE_DIR.exists():
    ...
    for csv_file in BASE_DIR.glob("*_CoverageScheme.csv"):  # Match {year}_{week}_CoverageScheme.csv
        file_parts = csv_file.stem.split('_')
        if len(file_parts) >= 2 and file_parts[-1] == "CoverageScheme":
            try:
