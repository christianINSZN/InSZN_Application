#!/usr/bin/env python3
"""
CFB Enhanced Model - FAST VERSION
- Uses only weeks 11-12 for testing (since 13-15 aren't completed yet)
- Lighter models for faster training
"""
...
# ============================================================================
# CONFIG - ADJUSTED FOR AVAILABLE DATA
# ============================================================================
DB_FILE = Path("/Users/christianberry/Desktop/Perennial Data/perennial-data-app/server/data/db/cfb_database.db")

YEAR = 2025
TRAIN_WEEKS = range(1, 11)  # Weeks 1-10
TEST_WEEKS = [11, 12]  # Only weeks that have completed games!

HOME_ADV = 2.5
ELO_K = 32

print("="*80)
print("CFB ENHANCED MODEL - FAST VERSION")
print("="*80)
print(f"Started at: {datetime.now().strftime('%H:%M:%S')}")
print(f"Train: Weeks {list(TRAIN_WEEKS)[0]}-{list(TRAIN_WEEKS)[-1]}")
...
def load_year_data(year):
    log_progress("Loading game data...")
    ...
    log_progress(f"  Loaded {len(games)} FBS games")
...
# Simplified PFF (skip for speed if needed)
pff_cache = {}

def get_team_pff_grades_simple(team_id, week, year):
    """Simplified PFF - only most important grades"""
    ...
    try:
        # Just get overall offensive and defensive grades
...
# FASTER MODEL - Single LightGBM instead of ensemble
log_progress("Training LightGBM model (faster than ensemble)...")
...
if len(completed) > 0:
    ...
    for week in TEST_WEEKS:
        ...
        if len(week_preds) > 0:
            ...
            print(f"   Week {week}: {len(week_preds)} games → {week_acc:.1f}% accuracy")

# Save
cursor = conn.cursor()
cursor.execute("DROP TABLE IF EXISTS Teams_Games_Enhanced_Predictions")
cursor.execute("""
    CREATE TABLE Teams_Games_Enhanced_Predictions (
...
cursor.executemany("""
    INSERT OR REPLACE INTO Teams_Games_Enhanced_Predictions VALUES (?, ?, ?, ?, ?, ?, ?, ?)
""", [(
