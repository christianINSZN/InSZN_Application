...
cursor.execute("PRAGMA table_info(Players_PPA_TE)")
columns = [col[1] for col in cursor.fetchall()]
if "teamID" not in columns:
    cursor.execute("ALTER TABLE Players_PPA_TE ADD COLUMN teamID TEXT")

# Create Players_PPA_RB table if it doesn't exist
cursor.execute("""
CREATE TABLE IF NOT EXISTS Players_PPA_TE (
...
def save_ppa_data(year, position):
    ...
    for player in data:
        ...
        cursor.execute(
            """
            INSERT OR REPLACE INTO Players_PPA_TE (
