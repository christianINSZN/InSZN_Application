...
cursor.execute("PRAGMA table_info(Players_PPA_RB)")
columns = [col[1] for col in cursor.fetchall()]
if "teamID" not in columns:
    cursor.execute("ALTER TABLE Players_PPA_RB ADD COLUMN teamID TEXT")

# Create Players_PPA_RB table if it doesn't exist
cursor.execute("""
CREATE TABLE IF NOT EXISTS Players_PPA_RB (
...
def save_ppa_data(year, position):
    ...
    for player in data:
        ...
        cursor.execute(
            """
            SELECT prgs.attempts, pb.teamID
            FROM Players_RushingGrades_Season prgs
        ...
        rushing_attempts = result[0] if result and result[0] is not None else 0
        team_id = result[1] if result and result[1] is not None else None
        min_rushing_threshold_hit = rushing_attempts >= MIN_RUSHING_SNAPS

        cursor.execute(
            """
            INSERT OR REPLACE INTO Players_PPA_RB (
