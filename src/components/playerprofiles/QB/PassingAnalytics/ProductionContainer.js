...
const PocketProduction = ({ playerId, year, weeklyGrades, teamGames, allPlayerPercentiles }) => {
  ...
  const handleCheckboxChange = async (metricId, selectedPlayerId) => {
    ...
    if (!checkedPlayers[metricId][selectedPlayerId]) {
      ...
      if (!newWeeklyData) {
        console.warn(`No valid data for player ${selectedPlayerId}, metric ${metricId}, reverting checkbox`);
        setCheckedPlayers(prev => ({
          ...prev,
          [metricId]: {
            ...prev[metricId],
  ...
  const opponentLookup = useMemo(() => {
    if (!teamGames || teamGames.length === 0) {
      console.warn('teamGames is empty or not iterable, using empty opponentLookup');
      return {};
    }
  ...
  }, [metrics, opponentLookup, teamGames, weeklyGrades, allPlayerPercentiles, playerId, checkedPlayers, playerWeeklyData]);
