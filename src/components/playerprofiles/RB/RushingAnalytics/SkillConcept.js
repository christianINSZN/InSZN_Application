...
const SkillConcept = ({ playerId, year, weeklyGrades, teamGames, allPlayerPercentiles }) => {
  const chartRefs = {
    yards_after_contact: useRef(null),
    breakaway_percent: useRef(null),
    breakaway_yards: useRef(null),
    elu_rush_mtf: useRef(null),
    elusive_rating: useRef(null),
  };
  ...
  const [showSearch, setShowSearch] = useState({
    yards_after_contact: false,
    breakaway_percent: false,
    breakaway_yards: false,
    elu_rush_mtf: false,
    elusive_rating: false,
  });
  const colors = [
  ...
  useEffect(() => {
    ...
    const metrics = [
      { id: 'yards_after_contact', field: 'yards_after_contact', title: 'Yards After Contact', min: 0, max: 200, unit: 'Yards After Contact' },
      { id: 'breakaway_percent', field: 'breakaway_percent', title: 'Breakaway (%)', min: 0, max: 100, unit: 'Breakaway (%)' },
      { id: 'breakaway_yards', field: 'breakaway_yards', title: 'Breakaway Yards', min: 0, max: 200, unit: 'Breakaway Yards' },
      { id: 'elu_rush_mtf', field: 'elu_rush_mtf', title: 'Missed Forced Tackles (Rushing)', min: 0, max: 20, unit: 'Missed Forced Tackles (Rushing)' },
      { id: 'elusive_rating', field: 'elusive_rating', title: 'Elusive Rating', min: 0, max: 300, unit: 'Elusive Rating' },
    ];
  ...
  const getTopPerformers = (metricField, searchTerm = '') => {
    ...
    return Object.entries(allPlayerPercentiles)
      ...
      .map(player => ({
        ...
        name: player.name,
        value: player.value.toFixed(metricField === 'yards_after_contact' || metricField === 'elu_rush_mtf' ? 2 : 1),
      }));
  };
  ...
  return (
    <div className="pocket-production-container grid grid-cols-[80%_18%] gap-4">
      {[
        { id: 'yards_after_contact', title: 'Yards After Contact', field: 'yards_after_contact' },
        { id: 'breakaway_percent', title: 'Breakaway (%)', field: 'breakaway_percent' },
        { id: 'breakaway_yards', title: 'Breakaway Yards', field: 'breakaway_yards' },
        { id: 'elu_rush_mtf', title: 'Missed Forced Tackles (Rushing)', field: 'elu_rush_mtf' },
        { id: 'elusive_rating', title: 'Elusive Rating', field: 'elusive_rating' },
      ].map((metric, index) => (
              ...
              <table className="w-full text-xs text-gray-500">
                <thead>
                  <tr className="border-b border-gray-200">
                    ...
                    <th className="text-right py-1">{metric.title === 'Yards After Contact' ? 'YAC' : metric.title === 'Breakaway (%)' ? 'Rate' : metric.title === 'Breakaway Yards' ? 'YDS' : metric.title === 'Missed Forced Tackles (Rushing)' ? 'MFT' : 'Rating'}</th>
