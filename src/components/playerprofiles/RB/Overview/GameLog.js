...
const GameLog = ({ teamGames, weeklyGrades, year }) => {
          ...
          {sortedGames.length > 0 ? (
            sortedGames.map((game, index) => {
              ...
              const data = {
                ...
                YPC: weekGrade['ypa'] !== undefined && weekGrade['ypa'] !== null ? weekGrade['ypa'].toFixed(1) : '-',
                TD: weekGrade['touchdowns'] !== undefined && weekGrade['touchdowns'] !== null ? weekGrade['touchdowns'] : '-',
                FUM: weekGrade['fumbles'] !== undefined && weekGrade['fumbles'] !== null ? weekGrade['fumbles'] : '-',
              };
              return (
                <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-[#235347]/20'}>
                  {columns.map(col => (
                    <td
                      ...
                      style={{ verticalAlign: 'middle', lineHeight: col.key === 'Date' || col.key === 'Opponent' || col.key === 'Score' ? '1' : '1.1', textAlign: col.align === 'middle' ? 'center' : 'left' }}
                    >
                      {data[col.key]}
                    </td>
                  ))}
            ...
            <tr>
              <td colSpan={isFullView ? 10 : 7} className="p-4 text-center text-gray-500 text-[10px] sm:text-xs">No game data available</td>
            </tr>
          )}
        </tbody>
      </table>
  ...
  return (
    <div className={isMobile ? "bg-gray-100 p-2 rounded-lg shadow-lg" : "bg-gray-100 p-0 rounded-lg shadow-lg"}>
      ...
      <div className={isMobile ? "h-auto overflow-x-auto relative" : "h-100 overflow-auto relative"}>
      ...
      {showComingSoon && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className={isMobile ? "bg-white p-4 rounded-lg shadow-xl w-[90%] h-[50%]" : "bg-white p-6 rounded-lg shadow-xl w-auto h-auto"}>
            <p className={isMobile ? "text-sm font-semibold text-black" : "text-lg font-semibold text-black"}>Game Recaps Coming Soon</p>
            <button
              className={isMobile ? "mt-2 px-3 py-1 bg-[#235347] text-white rounded hover:bg-[#1b3e32]" : "mt-4 px-4 py-2 bg-[#235347] text-white rounded hover:bg-[#1b3e32]"}
