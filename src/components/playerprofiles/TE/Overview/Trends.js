...
const Trends = ({ isPopupOpen, setIsPopupOpen, setSelectedGrade, selectedGrade, year, teamGames, weeklyGrades, weeklyBlockingGrades, className = "text-sm sm:text-base" }) => {
  ...
  const trendDataMemo = useMemo(() => {
    ...
    const gamesWithStats = teamGames
      .map(game => {
        ...
        const blockGrade = weeklyBlockingGrades[key] || {};
        const hasStats = (
          Object.keys(metrics).some(metric => 
            metric === 'grades_pass_block' 
              ? blockGrade[metric] !== undefined && blockGrade[metric] !== null
              : grade[metric] !== undefined && grade[metric] !== null
          )
        );
    ...
    Object.keys(metrics).forEach(metric => {
      const values = gamesWithStats
        .map(game => {
          const grade = metric === 'grades_pass_block' ? (weeklyBlockingGrades[game.key] || {}) : (weeklyGrades[game.key] || {});
    ...
    return { trendUp, trendDown };
  }, [teamGames, weeklyGrades, weeklyBlockingGrades]);

  useEffect(() => {
    setTrendData(trendDataMemo);
  }, [trendDataMemo]);
