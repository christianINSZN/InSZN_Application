...
const HeadlineGrades = ({
  ...
  useEffect(() => {
    ...
    if (isPopupOpen && selectedGrade) {
      ...
      const gradeToField = {
        ...
        'Pass Block Grade': 'grades_pass_block',
      ...
      const dataValues = sortedGames.map(game => {
        const key = `${game.week}_${game.seasonType}`;
        const weekData = gradeField === 'grades_pass_block' ? (weeklyBlockingGrades[key] || {}) : (weeklyGrades[key] || {});
      ...
      const chartOptions = isMobile
        ? {
            scales: {
              x: {
                title: { display: false, text: 'Opponent', font: { size: 10 } },
        ...
        : {
            scales: {
              x: { 
                ...
                ticks: { autoSkip: false, maxRotation: 45, minRotation: 45, labelOffset: 10, font: { size: 12 } }
              },
              y: { 
                title: { display: true, text: 'Grade', font: { size: 14 } }, 
                beginAtZero: true, 
                min: yMin, 
                max: yMax, 
                ticks: { stepSize: (yMax - yMin) / 5, font: { size: 12 } }
              },
            },
            plugins: { 
  ...
  }, [isPopupOpen, selectedGrade, weeklyGrades, weeklyBlockingGrades, teamGames, isMobile]);

  return (
    <div className={isMobile ? "h-auto bg-white rounded-lg shadow-lg" : "h-80 bg-white rounded-lg shadow-lg"}>
      <h2 className="flex items-center justify-center text-base sm:text-xl bg-[#235347] font-bold text-white shadow-lg border-b border-[#235347] h-8 sm:h-[40px] rounded">Headline Grades</h2>
      {isMobile ? (
        <div className="grid grid-cols-1 gap-4 mb-4 h-auto">
          ...
          <div className="bg-gray-0 p-2 rounded text-center hover:bg-[#235347]/20 shadow-lg min-h-[120px]" onClick={() => { setIsPopupOpen(true); setSelectedGrade('Pass Block Grade'); }}>
        ...
        <div className="grid grid-cols-3 gap-4 mb-4 h-[40%]">
          ...
          <div className="bg-gray-0 p-2 rounded text-center h-full hover:bg-[#235347]/20 shadow-lg" onClick={() => { setIsPopupOpen(true); setSelectedGrade('Pass Block Grade'); }}>
            <h3 className="text-sm font-medium">Pass Block</h3>
            <p className="text-4xl font-bold text-gray-800">{convertToLetterGrade(getGradeValue('Pass Block Grade'))}</p>
            <p className="text-[11px] text-gray-500 p-2.5">Percentile: {formatPercentile(getGradeValue('Pass Block Grade'))}</p>
          </div>
      ...
      {isPopupOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className={isMobile ? "bg-white p-2 rounded-lg shadow-lg w-[95%] h-[50%] flex flex-col" : "bg-white p-4 rounded-lg shadow-lg w-1/2 h-1/2 flex flex-col"} style={{ width, height }}>
            <h3 className={isMobile ? "text-sm font-semibold mb-1" : "text-lg font-semibold mb-2"}>{selectedGrade}</h3>
            <div className="flex-1 overflow-auto" style={{ maxHeight: isMobile ? '275px' : 'unset' }}>
