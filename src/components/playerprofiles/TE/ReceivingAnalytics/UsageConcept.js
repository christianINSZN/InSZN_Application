...
const PocketProduction = ({ playerId, year, weeklyGrades, teamGames, allPlayerPercentiles }) => {
  const chartRefs = {
    ...
    routes: useRef(null),
    yprr: useRef(null),
    slot_rate: useRef(null),
    wide_rate: useRef(null),
  };
  ...
  useEffect(() => {
    ...
    const metrics = [
      { id: 'targets', field: 'targets', title: 'Targets', min: 0, max: 20, unit: 'Targets' },
      { id: 'routes', field: 'routes', title: 'Routes Run', min: 0, max: 50, unit: 'Routes Run' },
      { id: 'yprr', field: 'yprr', title: 'Yards Per Route Run', min: 0, max: 50, unit: 'Yards Per Route Run' },
      { id: 'slot_rate', field: 'slot_rate', title: 'Slot Route Rate', min: 0, max: 100, unit: 'Slot Route Rate' },
      { id: 'wide_rate', field: 'wide_rate', title: 'Wide Route Rate', min: 0, max: 100, unit: 'Wide Route Rate' },
    ];
  ...
  const getTopPerformers = (metricField, searchTerm = '') => {
    ...
    return Object.entries(allPlayerPercentiles)
      ...
      .map(player => ({
        ...
        name: player.name,
        value: player.value.toFixed(metricField === 'targets' || metricField === 'slot_rate' ? 2 : 1),
      }));
  };
  ...
  return (
    <div className="pocket-production-container grid grid-cols-[80%_18%] gap-4">
      {[
        { id: 'targets', title: 'Targets', field: 'targets' },
        { id: 'routes', title: 'Routes Run', field: 'routes' },
        { id: 'yprr', title: 'Yards Per Route Run', field: 'yprr' },
        { id: 'slot_rate', title: 'Slot Route Rate', field: 'slot_rate' },
        { id: 'wide_rate', title: 'Wide Route Rate', field: 'wide_rate' },
      ].map((metric, index) => (
              ...
              <table className="w-full text-xs text-gray-500">
                <thead>
                  <tr className="border-b border-gray-200">
                    ...
                    <th className="text-right py-1">{metric.title === 'Targets' ? 'TAR' : metric.title === 'Routes Run' ? 'Routes' : metric.title === 'Yards Per Route Run' ? 'YPRR' : metric.title === 'Slot Route Rate' ? 'Rate' : 'Rate'}</th>
