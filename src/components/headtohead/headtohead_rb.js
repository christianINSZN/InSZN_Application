...
import HeadToHeadContainer from './positions/RB/HeadToHeadContainer';
import ContainerA from './positions/RB/ContainerA';
import ContainerB from './positions/RB/ContainerB';
    ...
    const fetchPlayerData = async (player, playerKey) => {
      ...
      try {
        const [gradesResponse, basicResponse, gamesResponse, percentilesResponse] = await Promise.all([
          ...
          fetch(`${process.env.REACT_APP_API_URL}/api/player_percentiles_RB/${player.playerId}/${player.year}`, {
            method: 'GET',
            headers: { 'Content-Type': 'application/json' },
          }),
        ]);
        ...
        const gradesPromises = uniqueGames.map(game => {
          const url = `${process.env.REACT_APP_API_URL}/api/player_rushing_weekly_all/${player.playerId}/${player.year}/${game.week}/${game.seasonType}`;
