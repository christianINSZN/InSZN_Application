...
const HeadToHeadContainer = ({ className, onPlayerDataChange, year }) => {
  ...
  const [year1, setYear1] = useState(() => localStorage.getItem('year1') || defaultYear);
    ...
    const fetchData1 = async () => {
      if (player1 && year1 && isMounted) {
        ...
        try {
          ...
          const statsResponse = await fetch(`${process.env.REACT_APP_API_URL}/api/player_percentiles_QB/${player1.value}/${year1}`);
